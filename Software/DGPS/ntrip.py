                ...
                if error_indicator==0:
                    ...
                    while data:
                        try:
                            ...
                            ser.write(data) # Output to UART serial port
...
#                            print datetime.datetime.now()-connectTime
                            if maxConnectTime :
                                if datetime.datetime.now() > connectTime+EndConnect:
                                    if self.verbose:
                                        print("Connection Timed exceeded\n")
                                    print ("EXCEEDED MAXCONNECTTIME")
                                    sys.exit(0)
...
if __name__ == '__main__':
    ...
    parser.add_option("-u", "--user", type="string", dest="user", default="awadaskar", help="The Ntripcaster username.  Default: %default")
    parser.add_option("-p", "--password", type="string", dest="password", default="nYgmwmg1", help="The Ntripcaster password. Default: %default")
    ...
    if options.org:
        ...
        # ntripArgs['caster']=args[0] #"rtgpsout.unavco.org"
        # ntripArgs['port']=int(args[1]) #2101
        # ntripArgs['mountpoint']=args[2] #"COPR_RTCM3"        
        ntripArgs['caster'] = "rtgpsout.unavco.org"
        ntripArgs['port'] = 2101
        ntripArgs['mountpoint'] = "COPR_RTCM3"
